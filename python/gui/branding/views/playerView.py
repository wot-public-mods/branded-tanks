...
class BrandingPlayerView(BrandingPlayerViewMeta):
	...
	def onSettings(self, teamFirst, teamSecond, onlyOnMyTank):
		g_controllers.processor.appendSettings({'teamFirst': teamFirst, 'teamSecond': teamSecond, \
											 'onlyOnMyTank': onlyOnMyTank })
		self.onWindowClose()
	
	def __generateLocalizationCtx(self):
		"""result represented by BrandingPlayerLocalizationVO"""
		return { 'windowTitle': l10n('ui.operator.windowTitle'), 'headerTextAlly': l10n('ui.player.headerTextAlly'), \
			'headerTextEnemy': l10n('ui.player.headerTextEnemy'), 'onlyMyTankLabel': l10n('ui.player.onlyMyTank.label'), \
